 ...
 * Options:
 *
 *   - `refreshTokenHeader`  header name where the username is found, defaults to 'x-refresh-token'
 *   - `tokenHeader`  header name where the token is found, defaults to 'x-token'
 *   - `refreshTokenField`  field name where the username is found, defaults to 'refresh-token'
 *   - `tokenField`  field name where the password is found, defaults to 'password'
 *   - `refreshTokenQuery`  query string name where the username is found, defaults to 'username'
...
function Strategy(options, verify) {
    ...
    this._refreshTokenHeader = options.usernameHeader ? options.usernameHeader.toLowerCase() : 'x-refresh-token';
    this._tokenHeader = options.tokenHeader ? options.tokenHeader.toLowerCase() : 'x-token';

    this._refreshTokenField = options.usernameField ? options.usernameField.toLowerCase() : 'refresh-token';
    this._tokenField = options.tokenField ? options.tokenField.toLowerCase() : 'token';

    this._refreshTokenQuery = options.usernameQuery ? options.usernameQuery.toLowerCase() : this._refreshTokenField;
...
Strategy.prototype.authenticate = function (req, options) {
    options = options || {};
    var self = this;
    var username = req.headers[this._refreshTokenHeader] || lookup(req.body, this._refreshTokenField) || lookup(req.query, this._refreshTokenQuery);
    var token = req.headers[this._tokenHeader] || lookup(req.body, this._tokenField) || lookup(req.query, this._tokenQuery);

    if (!username || !token) {
        return this.fail(new _Error.BadRequestError(400, { message: 'Missing credentials' }));
    }
